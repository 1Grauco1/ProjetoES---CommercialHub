...
from app.schemas.proprietario_schemas import (ProprietarioCreate,
...
async def buscar(id: int, db=Depends(get_db)):
    prop = proprietario_crud.buscar_proprietario_id(db, id)
    if not prop:
        raise HTTPException(status_code=404, detail="Proprietário não encontrado")
    return prop

@router.post("/", response_model=ProprietarioResponse)
async def criar(dados: ProprietarioCreate, db=Depends(get_db), user=Depends(get_current_user)):
